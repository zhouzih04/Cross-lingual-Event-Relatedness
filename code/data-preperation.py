...
def load_labeled_data(
    ...
    Args:
        ...
        language_filter: Filter by language ('en', 'zh', or None for all)
    """
    ...
    if language_filter:
        data = [t for t in data if t.get('language', 'en') == language_filter]
        print(f"Filtered to language='{language_filter}'")
    ...
    for topic in data:
        ...
        lang_counts[lang] += 1
    print(f"Topics by language: {dict(lang_counts)}")
...
def generate_pairwise_data(
    ...
    for topic in topics:
        ...
        topic_lang = topic.get('language', 'en')
    ...
    for topic_a, topic_b in topic_pairs:
        ...
        lang_a = topic_a.get('language', 'en')
        lang_b = topic_b.get('language', 'en')
        ...
        for art_a in sample_a:
            for art_b in sample_b:
                # For negative pairs, use language of topic_a (arbitrary choice)
                # In practice both should be same language when training within-language
                negative_pairs.append({
                    ...
                    'node_a': art_a['node_index'],
                    'node_b': art_b['node_index'],
                    'language': lang_a if lang_a == lang_b else 'mixed'
                })
...
def prepare_all_datasets(
    ...
    output_dir: str = '../output/prepared_data',
    negative_ratio: float = 1.0,
    random_seed: int = 42,
    language_filter: str = None
    ...
    Args:
        ...
        random_seed: For reproducibility
        language_filter: Filter by language ('en', 'zh', or None for all/multilingual)
    ...
    # Load data with optional language filter
    topics = load_labeled_data(labeled_filepath, language_filter=language_filter)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--output', type=str, default='../output/prepared_data',
    ...
    parser.add_argument('--language', type=str, default=None,
                        choices=['en', 'zh', None],
                        help='Filter by language (en=English, zh=Chinese, None=all/multilingual)')

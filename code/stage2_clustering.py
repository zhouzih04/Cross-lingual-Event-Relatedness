...
def evaluate_clustering_per_language(
    articles: pd.DataFrame,
    predicted_clusters: np.ndarray,
    true_labels: np.ndarray
) -> Dict[str, Dict[str, float]]:
    """
    Evaluate clustering quality separately for each language.

    Args:
        articles: DataFrame with 'language' column
        predicted_clusters: Predicted cluster assignments
        true_labels: True topic labels
    ...
    if 'language' not in articles.columns:
        print("Warning: 'language' column not found in articles. Returning overall metrics only.")
        return results
    ...
    for lang in languages:
        ...
        lang_mask = articles['language'] == lang
        lang_indices = np.where(lang_mask.values)[0]
        n_samples = len(lang_indices)

        if n_samples == 0:
            continue

        print(f"\n  [{lang.upper()}] Evaluating on {n_samples} articles...")

        # Filter predictions and labels for this language
        pred_lang = predicted_clusters[lang_indices]
        true_lang = true_labels[lang_indices]

        # Filter out noise (-1) for evaluation
        valid_mask = pred_lang != -1
        pred_filtered = pred_lang[valid_mask]
        true_filtered = true_lang[valid_mask]

        if len(pred_filtered) == 0:
            print(f"    No valid clusters found for {lang}")
            continue
        ...
        metrics = {
            'n_samples': n_samples,
            'n_valid': int(len(pred_filtered)),
            'n_clusters': int(len(set(pred_filtered))),
            'ari': float(ari),
            'nmi': float(nmi),
            'bcubed_precision': float(bcubed['bcubed_precision']),
            'bcubed_recall': float(bcubed['bcubed_recall']),
            'bcubed_f1': float(bcubed['bcubed_f1'])
        }
    ...
    print(f"\n  [ALL] Evaluating on {len(articles)} articles...")
    mask = predicted_clusters != -1
    pred_filtered = predicted_clusters[mask]
    true_filtered = true_labels[mask]
    ...
    print(f"    ARI: {results['all']['ari']:.4f}")
    print(f"    NMI: {results['all']['nmi']:.4f}")
    print(f"    BCubed F1: {results['all']['bcubed_f1']:.4f}")
